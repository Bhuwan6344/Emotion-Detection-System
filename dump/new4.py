...
MODEL_FILE = '/Users/sam/Desktop/7th sem/web-app/backend/models/model_lessparameter,55E.h5'
CSV_FILE = 'static/csv/history_lessparameter,55E.csv'
IMAGE_DIR = '/Users/sam/Downloads/Dataset/'
...
def generate_report(model, csv_file, image_dir):
    train = pd.DataFrame()
    test = pd.DataFrame()
    train['image'], train['label'] = createdataframe(os.path.join(image_dir, 'train'))
    test['image'], test['label'] = createdataframe(os.path.join(image_dir, 'test'))
...
@app.route('/graph1/')
def graph():
    graph_html = generate_graph(CSV_FILE)
    train_report, test_report, train_matrix, test_matrix = generate_report(model, CSV_FILE, IMAGE_DIR)
